...
import { clearSession } from './utils/setup';
...
if (!volunteer_email || !volunteer_password || !charity_email || !charity_password) {
  throw new Error('Missing TEST_USER_EMAIL_VOLUNTEER, TEST_USER_PASSWORD_VOLUNTEER, TEST_USER_EMAIL_CHARITY, or TEST_USER_PASSWORD_CHARITY in environment variables');
}
...
test.describe('Task Management Flows', () => {
  test.describe('Volunteer Flow', () => {
    ...
    test('1. should display task details', async ({ page }) => {
      ...
      await page.getByTestId('dashboard-nav').getByRole('link', { name: 'Tasks' }).click();
      // Click and verify specific task details

      await page.getByRole('button', { name: 'Website Redesign for Non-' }).click();
      
      // Verify task content is visible
      await expect(page.getByTestId('banner-items').getByText('Website Redesign for Non-')).toBeVisible();
      await expect(page.getByTestId('banner-items').getByText('INCOMPLETE')).toBeVisible();
      await expect(page.getByTestId('banner-items').getByText('Saving Seas')).toBeVisible();

      // Verify task metadata
      expect(page.locator('div').filter({ hasText: /^Team Size1volunteer needed$/ }).locator('div').first()).toBeVisible();
    ...
    test.describe.serial('Application Flow', () => {
      test('2. should apply to task', async ({ page }) => {
        ...
        await page.getByRole('button', { name: 'Fund raising LOW Community' }).click();
        await page.getByRole('button', { name: 'volunteer for task' }).click();
        await expect(page.getByText('Application submitted')).toBeVisible();
        await expect(page.getByRole('button', { name: 'withdraw from task' })).toBeVisible();
        ...
        await page.getByRole('button', { name: 'Fund raising Due: 2/23/2025' }).click();

        await expect(page.getByRole('button', { name: 'withdraw' })).toBeVisible();
      });

      test('3. should withdraw task application', async ({ page }) => {
        await page.getByTestId('dashboard-nav').getByRole('link', { name: 'Dashboard' }).click();
        await page.getByRole('link', { name: 'Fund raising PENDING' }).click();
        await page.getByRole('button', { name: 'applications' }).click();
        ...
        await expect(page.getByRole('button', { name: 'reapply' })).toBeVisible();
      });

      test('4. should delete task application', async ({ page }) => {
        await page.getByTestId('dashboard-nav').getByRole('link', { name: 'Dashboard' }).click();
        await page.getByRole('link', { name: /Fund raising (WITHDRAWN|REJECTED|PENDING)/i }).click();
        await page.getByRole('button', { name: 'applications' }).click();
        await page.getByRole('button', { name: 'Fund raising Due: 2/23/2025' }).click();
        await page.getByRole('button', { name: 'delete-application' }).click();
        await expect(page.getByRole('button', { name: 'Fund raising Due: 2/23/2025' })).not.toBeVisible();
      });
    });
  ...
  test.describe('Charity Flow', () => {
    ...
    test.describe.serial('Task Creation and Management', () => {
      test('1. should create task', async ({ page }) => {
        ...
        await page.getByTestId('dashboard-nav').getByRole('link', { name: 'Manage Tasks' }).click();
        await page.getByRole('button', { name: 'create-task' }).click();

        // Fill in task details
        await page.getByRole('textbox', { name: 'title' }).click();
        await page.getByRole('textbox', { name: 'title' }).fill('Build an Accessible Website for a Animal Shelter');

        // Add impact
        await page.getByRole('textbox', { name: 'impact' }).click();
        await page.getByRole('textbox', { name: 'impact' }).fill('This task will help the animal shelter reach more potential');

        // Add description
        await page.getByRole('textbox', { name: 'description' }).click();
        await page.getByRole('textbox', { name: 'description' }).fill('Develop a user-friendly and accessible website with pet adoption listings, donation options, and volunteer sign-up forms');

        // Add technical skills
        await page.getByRole('textbox', { name: 'Enter a technical Skill (4' }).fill('W');
        await page.getByRole('button', { name: 'AWS' }).click();

        // Select category
        await page.getByRole('textbox', { name: 'Enter the charitable category' }).fill('ANIM');
        ...
        // Set deadline
        await page.getByRole('textbox', { name: 'deadline' }).fill('2025-03-27');

        // Add deliverables
        await page.getByRole('textbox', { name: 'Enter a deliverable (5' }).fill('Fully functional website with adoption listings');
        await page.locator('div').filter({ hasText: /^DeliverablesAdd$/ }).getByRole('button').click();
        ...
        await page.getByRole('button', { name: 'Build an Accessible Website' }).click();

        await expect(
          page.locator('p[data-testid="banner-item-value"][title="Build an Accessible Website for a Animal Shelter"]')
        ).toBeVisible();
      ...
      test('2. should display task details', async ({ page }) => {
        ...
        // Click and verify specific task details
        await page.getByRole('button', { name: 'Homelessness Due: 3/2/2025' }).click();

        // Verify task content is visible
        await expect(page.getByTestId('banner-items').getByText('Homelessness')).toBeVisible();
        await expect(page.getByText('Reduce homelessness')).toBeVisible();
        await expect(page.getByText('Provide shelter for')).toBeVisible();

        // Verify task metadata
        await expect(page.getByText('Team Size2volunteers needed')).toBeVisible();
      ...
      test('3. should perform task application actions', async ({ page }) => {
        ...
        await expect(page.getByTestId('application-status')).toHaveText('ACCEPTED');
      ...
      test('4. should filter tasks by text search', async ({ page }) => {
        ...
        await expect(page.getByTestId('banner-items').getByText('Fund raising')).toBeVisible();
        await expect(page.getByRole('button', { name: 'Homelessness Due: 3/2/2025' })).not.toBeVisible();

        // Clear filter and verify all tasks are shown again
        await page.getByRole('textbox', { name: 'Search' }).nth(1).fill('');
        await expect(page.getByRole('button', { name: 'Homelessness Due: 3/2/2025' })).toBeVisible();
        await expect(page.getByRole('button', { name: 'Fund raising Due: 2/23/2025' })).toBeVisible();
      });
