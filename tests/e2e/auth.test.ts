...
if (!valid_email || !valid_password) {
  throw new Error('Missing TEST_USER_EMAIL_VOLUNTEER or TEST_USER_PASSWORD_VOLUNTEER in environment variables');
}
test.describe('Authentication with Zitadel', () => {
  ...
  test('should show error on invalid email', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Login Name' }).fill('incorrect@live.co.uk');
  ...
  test('should log out successfully', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Donate your digital skills' })).toBeVisible();
