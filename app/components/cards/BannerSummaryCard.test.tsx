...
describe('BannerSummaryCard', () => {
  ...
  describe('welcome section', () => {
    it('should display welcome section when showWelcome is true', () => {
      ...
      expect(screen.getByTestId('date-display')).toHaveTextContent('2024-01-01');
    });
  ...
  describe('banner items', () => {
    it('should render all banner items with correct content', () => {
      ...
      mockBannerItems.forEach((item, index) => {
        ...
        expect(within(bannerItem).getByTestId('banner-item-title')).toHaveTextContent(item.title);
        expect(within(bannerItem).getByTestId('banner-item-value')).toHaveTextContent(item.value);
      });
    });
